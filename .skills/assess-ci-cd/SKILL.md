...
If the first non-flag argument after the slash command looks like a path or glob (e.g., `/assess-ci-cd src/auth/` or `/assess-ci-cd terraform/`), restrict the autoexplore to that path. Treat everything else in the prompt as additional context.

If no path is provided AND intake is not triggered, after the first short response include a one-liner reminder: *"I'm reviewing the entire codebase. You can scope a future run with `/assess-ci-cd <path>`."*
...
**Specific to this skill:**
   ...
   5. ¿Plataforma CI? (GitHub Actions / GitLab CI / Jenkins / CircleCI)
   6. ¿Target de deploy? (k8s / EC2 / Lambda / Vercel / on-prem)
