...
If the first non-flag argument after the slash command looks like a path or glob (e.g., `/assess-testing src/auth/` or `/assess-testing terraform/`), restrict the autoexplore to that path. Treat everything else in the prompt as additional context.

If no path is provided AND intake is not triggered, after the first short response include a one-liner reminder: *"I'm reviewing the entire codebase. You can scope a future run with `/assess-testing <path>`."*
...
**Specific to this skill:**
   ...
   5. ¿Coverage aproximada actual?
   6. ¿Cuál nivel del pyramid es el más débil hoy? (unit / integration / E2E)
