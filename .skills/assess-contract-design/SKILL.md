...
If the first non-flag argument after the slash command looks like a path or glob (e.g., `/assess-contract-design src/auth/` or `/assess-contract-design terraform/`), restrict the autoexplore to that path. Treat everything else in the prompt as additional context.

If no path is provided AND intake is not triggered, after the first short response include a one-liner reminder: *"I'm reviewing the entire codebase. You can scope a future run with `/assess-contract-design <path>`."*
...
**Specific to this skill:**
   ...
   5. ¿Tipo de contrato? (REST / gRPC / GraphQL / async-eventos)
   6. ¿API pública o interna? ¿Hay consumers externos versionados?
