...
If the first non-flag argument after the slash command looks like a path or glob (e.g., `/assess-secure-coding src/auth/` or `/assess-secure-coding terraform/`), restrict the autoexplore to that path. Treat everything else in the prompt as additional context.

If no path is provided AND intake is not triggered, after the first short response include a one-liner reminder: *"I'm reviewing the entire codebase. You can scope a future run with `/assess-secure-coding <path>`."*
...
**Specific to this skill:**
   ...
   5. ¿El sistema maneja PHI / PII / NPI / cardholder data?
   6. ¿Aplican regulaciones específicas? (HIPAA, GDPR, LGPD, PCI-DSS, etc.)
