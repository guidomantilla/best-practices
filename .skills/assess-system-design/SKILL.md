...
If the first non-flag argument after the slash command looks like a path or glob (e.g., `/assess-system-design src/auth/` or `/assess-system-design terraform/`), restrict the autoexplore to that path. Treat everything else in the prompt as additional context.

If no path is provided AND intake is not triggered, after the first short response include a one-liner reminder: *"I'm reviewing the entire codebase. You can scope a future run with `/assess-system-design <path>`."*
...
**Specific to this skill:**
   ...
   5. ¿Tamaño actual? (monolito, N servicios)
   6. ¿Picos de tráfico esperados o scaling concerns concretos?
