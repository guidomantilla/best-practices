...
If the first non-flag argument after the slash command looks like a path or glob (e.g., `/assess-iac src/auth/` or `/assess-iac terraform/`), restrict the autoexplore to that path. Treat everything else in the prompt as additional context.

If no path is provided AND intake is not triggered, after the first short response include a one-liner reminder: *"I'm reviewing the entire codebase. You can scope a future run with `/assess-iac <path>`."*
...
**Specific to this skill:**
   ...
   5. ¿Cloud provider? (AWS / GCP / Azure / multi-cloud)
   6. ¿IaC tool y orquestador? (Terraform / Pulumi / CDK / K8s / Helm)
